...
import iotAbi from '../../iotcontract.json';
const contractAddress = '0x191D0Fe3298Eb5B17509969b9Bf6adbaB4D86801'; // 替换为实际的智能合约地址
...
const iotDevicesAddressDisplayDiv = document.getElementById('iotDevicesAddressDisplay');
...
const registerDeviceStatus = document.getElementById(
  'registerDeviceStatus',
);//jiawei
const transferDeviceStatus = document.getElementById(
  'transferDeviceStatus',
);//jiawei
const originMqttMessage = document.getElementById(
  'originMqttMessage',
);//jiawei
const signSendResult = document.getElementById(
  'signSendResult',
);//jiawei
...
const getEncryptionPublicKeyButton = document.getElementById(
  'getEncryptionPublicKeyButton',
);//jiawei
const encryptMessageInput = document.getElementById('encryptMessageInput');
const registerNewDeviceInput = document.getElementById('registerNewDeviceInput');//jiawei
const transferDeviceInput = document.getElementById('transferDeviceInput');//jiawei
const connectToMqttDeviceAddressInput = document.getElementById('connectToMqttDeviceAddressInput');//jiawei
const mqttDeviceOperatorsInput = document.getElementById('mqttDeviceOperatorsInput');//jiawei
const transferDeviceNewOwnerInput = document.getElementById('transferDeviceNewOwnerInput');//jiawei
const encryptButton = document.getElementById('encryptButton');
const setIntoSmartContractButton = document.getElementById('setIntoSmartContractButton');//jiawei
const registerNewDeviceButton = document.getElementById('registerNewDeviceButton');//jiawei
const transferDeviceButton = document.getElementById('transferDeviceButton');//jiawei
const connectMqttButton = document.getElementById('connectMqttButton');//jiawei
const singSendButton = document.getElementById('singSendButton');//jiawei
const decryptButton = document.getElementById('decryptButton');
const mqttDecryptButton = document.getElementById('mqttDecryptButton');
const encryptionKeyDisplay = document.getElementById('encryptionKeyDisplay');
const encryptionPublicKeyDisplay = document.getElementById('encryptionPublicKeyDisplay');
...
const initialize = async () => {
  try {
    ...
    iotContract = new ethers.Contract(contractAddress, iotAbi, ethersProvider.getSigner());// Create a new instance of the `ethers.Contract` class with the specified contract address, ABI, and signer
  ...
  const accountButtons = [
    ...
    getEncryptionPublicKeyButton,
    encryptMessageInput,
    registerNewDeviceInput,//jiawei
    transferDeviceInput,//jiawei
    connectToMqttDeviceAddressInput,//jiawei
    transferDeviceNewOwnerInput,//jiawei
    encryptButton,
    setIntoSmartContractButton,//jiawei
    registerNewDeviceButton,//jiawei
    transferDeviceButton,//jiawei
    connectMqttButton,//jiawei
    singSendButton,
    mqttDecryptButton,//jiawei
    mqttDeviceOperatorsInput,//jiawei
    decryptButton,
    ethSign,
    personalSign,
    personalSignVerify,
  ...
  const clearTextDisplays = () => {
    ...
    encryptionPublicKeyDisplay.innerText = '';
    encryptMessageInput.value = '';
    registerNewDeviceInput.value = '';
    transferDeviceNewOwnerInput.value = '';
    transferDeviceInput.value = '';
    connectToMqttDeviceAddressInput.value = '';
    mqttDeviceOperatorsInput.value = '';
  ...
  const updateButtons = () => {
    ...
    if (accountButtonsDisabled) {
      ...
      registerNewDeviceInput.disabled = false;//jiawei
      transferDeviceInput.disabled = false;//jiawei
      transferDeviceNewOwnerInput.disabled = false;//jiawei
      getEncryptionPublicKeyButton.disabled = false;//jiawei
      connectToMqttDeviceAddressInput.disabled = false;//jiawei
  ...
  const initializeAccountButtons = () => {
    ...
    getEncryptionPublicKeyButton.onclick = async () => {
      try {
        encryptionPublicKeyDisplay.innerText = await ethereum.request({
          method: 'eth_getEncryptionPublicKey',
          params: [accounts[0]],
        })
        setIntoSmartContractButton.disabled = false;
        ;
      } catch (error) {
        encryptionPublicKeyDisplay.innerText = `Error: ${error.message}`;
        setIntoSmartContractButton.disabled = true;
      }
    };

    setIntoSmartContractButton.onclick = () => {
      try {
        console.log(typeof (encryptionPublicKeyDisplay.innerText))
        console.log(encryptionPublicKeyDisplay.innerText)
        setPublicKey(encryptionPublicKeyDisplay.innerText.toString())
      } catch (error) {
        encryptionPublicKeyDisplay.innerText = `Error: ${error.message}`;
      }
    };
    ...
    registerNewDeviceButton.onclick = async () => {
      registerDeviceStatus.innerHTML = 'Registration In Progress';
      try {
        await registerNewDevice(registerNewDeviceInput.value);
        console.log(typeof (accountsDiv.innerText.toString()));
        getIotDevices(accountsDiv.innerText.toString()); //jiawei
        registerDeviceStatus.innerHTML = 'Registration Successful';
      } catch (error) {
        registerDeviceStatus.innerHTML = 'The Device Has Been Registered';
        throw error;
        //encryptionPublicKeyDisplay.innerText = `Error: ${error.message}`;
      }
    };

    transferDeviceButton.onclick = async () => {
      transferDeviceStatus.innerHTML = 'Transfer In Progress';
      try {
        await transferDevice(transferDeviceNewOwnerInput.value, transferDeviceInput.value);
        console.log(typeof (transferDeviceNewOwnerInput.value));
        getIotDevices(accountsDiv.innerText.toString());
        transferDeviceStatus.innerHTML = 'Transfer Successful';
      } catch (error) {
        transferDeviceStatus.innerHTML = 'Transfer Failure';
        throw error;
        //encryptionPublicKeyDisplay.innerText = `Error: ${error.message}`;
      }
    };//jiawei

    connectMqttButton.onclick = async () => {
      originMqttMessage.innerHTML = 'Connecting to device In Progress';
      try {
        connectToTopic(connectToMqttDeviceAddressInput.value);
        mqttDecryptButton.disabled = false;
        mqttDeviceOperatorsInput.disabled = false;//jiawei
      } catch (error) {
        originMqttMessage.innerHTML = 'Connecting Failure';
        mqttDecryptButton.disabled = true;
        throw error;
        //encryptionPublicKeyDisplay.innerText = `Error: ${error.message}`;
      }
    };//jiawei

    singSendButton.onclick = async () => {
      signSendResult.innerHTML = 'Signature & Send In Progress';
      const message = mqttDeviceOperatorsInput.value;
      try {
        const from = accounts[0];
        const msg = `0x${Buffer.from(message, 'utf8').toString('hex')}`
        const nonce = ethers.utils.hexlify(ethers.utils.randomBytes(64));
        const timestamp = Math.floor(Date.now() / 1000);
        const timestampHex = "0x" + timestamp.toString(16);
        const join = msg + nonce.substring(2) + timestampHex.substring(2);
        const jointest = msg + nonce + timestampHex;
        console.log("msg:"+msg)
        ...
        const sign = await ethereum.request({
          ...
          params: [join, from, 'Example password'],
        });
        const payload = JSON.stringify({ message, nonce, timestamp, sign });
        const obj = JSON.parse(payload);
        sendMessage(connectToMqttDeviceAddressInput.value,payload);
        signSendResult.innerText = `\nmessage: ${obj.message}\nnonce: ${obj.nonce}\ntimestamp: ${obj.timestamp}\nsignature: ${obj.sign}`;        singSendButton.disabled = false;
      } catch (error) {
        signSendResult.innerHTML = 'Signature & Send Failure';
        singSendButton.disabled = true;
        throw error;
        //encryptionPublicKeyDisplay.innerText = `Error: ${error.message}`;
      }
    };//jiawei
    ...
    function sendMessage(topic,message) {
      if (!mqttClient) {
        alert('Please connect to a topic first');
        return;
      }
      if (!message) {
      ...
      document.getElementById('mqttDeviceOperatorsInput').value = '';
    }
    ...
    function connectToTopic(topic) {
      ...
      mqttClient.on('message', (topic, message, packet) => {
        ...
        originMqttMessage.innerText = message.toString()
        //li.appendChild(document.createTextNode(topic + ': ' + message.toString()));
    ...
    registerNewDeviceInput.onkeyup = () => {
      console.log(registerNewDeviceInput.value.length)
      if (
        connectButton.disabled &&
        registerNewDeviceInput.value.length == 42
      ) {
        if (registerNewDeviceButton.disabled) {
          registerNewDeviceButton.disabled = false;
        }
      } else if (!registerNewDeviceButton.disabled) {
        registerNewDeviceButton.disabled = true;
      }
    };

    connectToMqttDeviceAddressInput.onkeyup = () => {
      console.log(connectToMqttDeviceAddressInput.value.length)
      if (
        connectButton.disabled &&
        connectToMqttDeviceAddressInput.value.length == 42
      ) {
        if (connectMqttButton.disabled) {
          connectMqttButton.disabled = false;
        }
      } else if (!connectMqttButton.disabled) {
        connectMqttButton.disabled = true;
      }
    };

    mqttDeviceOperatorsInput.onkeyup = () => {
      console.log(mqttDeviceOperatorsInput.value.length)
      if (
        connectButton.disabled &&
        mqttDeviceOperatorsInput.value.length > 0
      ) {
        if (singSendButton.disabled) {
          singSendButton.disabled = false;
        }
      } else if (!singSendButton.disabled) {
        singSendButton.disabled = true;
      }
    };


    transferDeviceInput.onkeyup = () => {
      updateTransferDeviceButtonStatus();
    };

    transferDeviceNewOwnerInput.onkeyup = () => {
      updateTransferDeviceButtonStatus();
    };


    function updateTransferDeviceButtonStatus() {
      if (
        connectButton.disabled &&
        transferDeviceInput.value.length == 42 &&
        transferDeviceNewOwnerInput.value.length == 42
      ) {
        if (transferDeviceButton.disabled) {
          transferDeviceButton.disabled = false;
        }
      } else if (!transferDeviceButton.disabled) {
        transferDeviceButton.disabled = true;
      }
    }
  ...
  mqttDecryptButton.onclick = async () => {
    try {
      decryptedMqttMessage.innerText = await ethereum.request({
        method: 'eth_decrypt',
        params: [originMqttMessage.innerText, ethereum.selectedAddress],
      });
    } catch (error) {
      decryptedMqttMessage.innerText = `Error: ${error.message}`;
    }
  };
  ...
  function handleNewAccounts(newAccounts) {
    ...
    getPublicKey(accounts);//jiawei
    console.log(typeof (accounts[0]))
    getIotDevices(accounts[0]);//jiawei
  ...
  function handleNewPublicKey(publicKey) {

    if (publicKey === '') {
      publicKeyDiv.innerHTML = "Please Set MQTT Encryption PublicKey";
    } else {
      publicKeyDiv.innerHTML = publicKey;
    }
  }

  async function getPublicKey(accounts) {
    try {
      console.log(accounts)
      console.log(typeof (accounts[0]))
      const publicKey = await iotContract.callStatic.getPublicKey(accounts[0]);
      console.log(typeof (publicKey))
      console.log(publicKey.length)
      handleNewPublicKey(publicKey);
    } catch (err) {
      console.error(err);
    }
  }//jiawei

  function handleNewIotDevices(allDevices) {

    if (allDevices.length === 0) {
      iotDevicesAddressDisplay.innerHTML = "You haven't registered any IOT devices with the blockchain";
    } else {
      const he = require("he");

      const allDevicesEncoded = allDevices.map(deviceAddress => he.encode(deviceAddress));
      iotDevicesAddressDisplay.innerHTML = allDevicesEncoded.join("<br>");
    }
  }

  async function getIotDevices(accountAddress) {
    try {
      console.log(accountAddress)
      console.log(typeof (accountAddress))
      const deviceCount = await iotContract.callStatic.balanceOf(accountAddress);
      console.log(typeof (deviceCount))
      console.log(deviceCount)
      console.log(typeof (deviceCount.toNumber()))
      console.log(deviceCount.toNumber())
      const allDevices = [];
      for (let i = 0; i < deviceCount; i++) {
        const deviceAddress = await iotContract.callStatic.ownerToDevices(accountAddress, i);
        allDevices.push(deviceAddress);
      }
      console.log(typeof (allDevices))
      console.log(allDevices)
      handleNewIotDevices(allDevices)
    } catch (err) {
      console.error(err);
    }
  }//jiawei
  ...
  async function setPublicKey(publicKey) {
    try {
      console.log(typeof (publicKey))
      console.log(publicKey.length)
      const tx = await iotContract.setPublicKey(publicKey);
      await tx.wait();
      handleNewPublicKey(publicKey)
    } catch (error) {
      tokenAddress.innerHTML = 'Set Into Smart Contract Failed';
      throw error;
    }
  }//jiawei

  async function registerNewDevice(deviceAddress) {
    try {
      console.log(typeof (deviceAddress))
      console.log("deviceAddress" + deviceAddress)
      const tx = await iotContract.createDevice(deviceAddress);
      await tx.wait();
    } catch (error) {
      registerDeviceStatus.innerHTML = error.message;
      throw error;
    }
  }//jiawei

  async function transferDevice(newOwnerAddress, deviceAddress) {
    try {
      console.log(typeof (deviceAddress))
      console.log("deviceAddress" + deviceAddress)
      console.log(typeof (newOwnerAddress))
      console.log("newOwnerAddress" + newOwnerAddress)
      const tx = await iotContract.transfer(newOwnerAddress, deviceAddress);
      await tx.wait();
    } catch (error) {
      transferDeviceStatus.innerHTML = error.message;
      throw error;
    }
  }//jiawei
